...
* `s.count()`: The `count()` method searches the substring in the given string and returns how many times the substring is present in it.
  * The syntax is: `string.count(substring, [start=...], [end=...])`
* `s.isdigit()`: Checks whether the string consists of digits only.
...
* `s.replace(old, new [, count])`: replaces each matching occurrence of the old character/text in the string with the new character/text.
  * Takes optional count argument which is the maximum number of times you want to replace the old substring with the new subsctring.
...
The actual implementation is pretty interesting - read about it [here](https://docs.python.org/3/faq/design.html#how-are-dictionaries-implemented-in-cpython).
...
Don't iterate using `if key in my_dict.keys()`. This is `O(n)`, compared to `O(1)` above.
...
Functional programming is commonly used with lambda functions. In Python 3, you need to `import functools`.
...
* Code outline: `functools.filter(function, iterable)`
...
* Code outline: `functools.map(function, iterable)`
...
Integer division can be done using `//`, i.e. the floor.
...
You can also use the `list.sort()` method. It modifies the list in-place (and **returns None** to avoid confusion). Usually it’s less convenient than `sorted()` - but if you don’t need the original list, it’s slightly more efficient.  Can also take the `reverse` and `key` parameter.
...
```
There are also [built-in](https://docs.python.org/3/library/functions.html) functions like `repr()` or `str()` that you can override for a class.
