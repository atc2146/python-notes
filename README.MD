...
* `s.join(list)`: The opposite of `split()`, joins the elements in the given list together using the string as the delimiter.
  * Example: `'---'.join(['aaa', 'bbb', 'ccc'])` -> `aaa---bbb---ccc`
* `string.count(substring, [start=...], [end=...])`: The `count()` method searches the substring in the given string and returns how many times the non-overlapping substring is present in it.
* `s.startswith()`: Returns True if a string starts with the specified prefix. If not, it returns False.
  * There is also the `endswith()` function.
* `s.find(substring, [start=...], [end=...])`: The `find()` method returns the index of first occurrence of the substring (if found). If not found, it returns -1.
* `s.index(substring, [start=...], [end=...])`: returns the index of a substring inside the string (if found). If the substring is not found, it raises an exception.
  * start and end (optional): substring is searched within str[start:end]
  * There is also `rindex()` which searches from the end
...
* `s.lower()` and `s.upper()`: changes to lowercase or uppercase, respectively.
  * There is also `s.capitalize()` which capitalizes the first character of the first word only in a sentence and lowercases the rest of the words. `s.title()` capitalizes the first character of all words in a sentence.
...
You can also [format](https://docs.python.org/3/tutorial/inputoutput.html#formatted-string-literals) inside the f-string
...
That is, the given end point is never part of the generated sequence.
...
### List Comprehension
Using listcomps can be useful if you want to build lists.

The general syntax is: *[expr for var in list {optional if expr}]*
...
* The `insert(index, x)` method inserts an element to the list at the specified index.
...
A list is a natural data structure to be used as a stack (LIFO). You can use `append()` and `pop()` calls to achieve this.

Also keep in mind that if you doing repeated `insert(0, x)` and `pop(0)` calls, you may want to consider using `collections.deque`. For a `list` those calls require the entire list to be shifted in memory.
...
Note: creating a list using `[]` is faster than `list()` since `list()` is a function call.
...
```python
my_dict = dict(bob=20, james=33, mary=18)
...
**Do not** iterate using `if key in my_dict.keys()`! This is `O(n)`, compared to `O(1)` above. Note technically for Python 3, `dict.keys()` returns a view, and `key in my_dict.keys()` actually calls the dict's hash lookup under the hood (`O(1)`), not a list scan.
...
# If we want 1 as the default value
...
A set has a significantly faster `in` search than lists.  This is because sets finds the element by computing a hash from the key - whereas, in general, the whole list needs to be searched.  Dict and set lookup both have `O(1)` time complexity, however.
...
2. `*args`
3. keyword only arguments: must be passed by name
3. `**kwargs`

```python
def example2(a, b, *args, kw1="default1", kw2="default2", **kwargs):
    print(f"a = {a}")
    print(f"b = {b}")
    print(f"args = {args}")
    print(f"kw1 = {kw1}")
...
example2(1, 2, 3, 4, kw2="fish", x=10)
# a = 1
# b = 2
# args = (3, 4)
# kw1 = default1
...
More formally, an iterator is an object that implements the iterator protocol:
  * `__iter__()` returns the iterator object itself.
  * `__next__()` returns the next element or raises `StopIteration` when exhausted.
 
  
That is, iterators implement the `__next__()` and `__iter__()` methods.
...
### Generators
Python generators are a way of creating iterators. In other words, a generator is a special kind of iterator created either with a generator function (using `yield`) or a generator expression (`(x for x in y)`).
...
Why Implement Generators?
  1. Easy to implement - less code than creating an iterator class
  2. Memory Efficient - only one item is in memory at a time
  3. Represent Infinite Stream - e.g., reading endless sensor data
  4. Pipelining Generators - chaining multiple generators without creating intermediate lists
...
Note, technically, they don’t actually modify code at compile time (Python is interpreted). They wrap functions at runtime when the function is defined.

Decorators are commonly used in areas such as authorization, logging, timing, authentication, caching, retry logic, and test performance.

Instead of assigning the function call to a variable, you can decorate a function with the @ symbol in Python.
...
1. When Python sees `@timer_decorator` above `slow_add`, it runs: `slow_add = timer_decorator(slow_add)`
2. `timer_decorator` returns the wrapper function, which adds timing logs around the original `slow_add`.
3. Calling `slow_add(5, 3)` actually calls `wrapper(5, 3)`, which logs the start, calls the original function, then logs the end.
...
These functions returns an iteratable. You can also pass in:
...
`math.comb(n, k)` is a function in Python that returns the number of ways to choose k items from n items without repetition and without order (i.e., combinations). It computes the binomial coefficient, commonly written as C(n, k) or n choose k.


```python
import math

math.comb(5, 2)  # Returns 10, since there are 10 ways to choose 2 items from 5
```

There is also a similar `math.perm(n, k)` function.
...
# Give a default value with type hints
def greeting(name: str = "John") -> str:
    return 'Hello ' + name
```

Use spaces around the `=` sign (PEP8) when combining an argument annotation with a default value `(align: bool = True)`.
...
There are some interesting ways to format strings. For example, there is the `str.format()` function.
...
Another common way is to use [f-strings](#f-strings).


See the Python Input Output [documentation](https://docs.python.org/3/tutorial/inputoutput.html) for more details.
...
### Hash tables
See the [dictionary](#dictionary) section.
...
* `list`: However, this can run into performance issues as it grows.
...
Breadth-First Search (BFS) is an algorithm that systematically explores a graph level by level

### Binary Search

This is provided by the `bisect` [module](https://docs.python.org/3/library/bisect.html).
...
Remember to import the package: `import pandas as pd`
